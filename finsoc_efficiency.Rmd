...
Where $Y_{it}$ represents the dependent variable, which takes efficiency scores derived from the data envelopment analysis (DEA) model. We compute three measure of efficiency that take turns as the dependent variables: social efficiency, financial efficiency, and socio-financial efficiency. In computing DEA, we follow the intermediation approach, where the inputs are net fixed assets, operating expenses to assets ratio, deposits to total assets ratio, liabilities and equity, and donations. The outputs comprise the per cent of female borrowers, average loan balance per borrower, gross loans to total assets ratio, and operational self-sufficiency (OSS). These input and output variables are described in more detail in section 5.1, together with the mechanics of the DEA model in part 5. $X_{it}$ on the other hand, it represents the set of independent variables as described in Table 1 below. 5.2 
...
kableExtra::kbl(caption = "Summary Statistics: DEA Model Input and Output Variables", 
...
We examine the extent to which MFIs reach the financially excluded sustainably. The first row measures financial efficiency with OSS as the output. The second row represents the social efficiency scores whose outputs are average loan balance and per cent of women borrowers. The third row also measures social efficiency and incorporates the breadth of outreach measured via gross loans to total assets. Row four measures socio-financial efficiency captured using operational self-sufficiency, average loan balance per borrower, and per cent of female borrowers. The last row measures socio-financial efficiency while also including breadth of outreach, gross loans, to total assets.

Table ()  below shows a summary of the efficiency scores. The results show  that the financial efficiency scores are heavily skewed to the right. Figure () visualizes the financial efficiency scores confirming the extent of the skewness. The results show that most MFIs are barely sustainable financially in spite of the paradigm shift towards commercialization. This observation begs the question; do commercially oriented legal forms of MFIs perform better than NGOs? Further, do newer MFIs that took root when donors put emphasis on the financial sustainability of MFIs do better financially than older MFIs? Panel C shows that only cooperatives and rural banks have higher median financial efficiency scores than NGOs. These results imply that it is possible for NGOs to combine the welfare model of reaching the financially excluded with financial sustainability. However, panel D shows that newer MFIs have higher financial sustainability scores relative to older MFIs. These results could mean that newer MFIs focus more on financial sustainability at the expense of financial exclusion or are better at balancing financial sustainability with social goals. 
